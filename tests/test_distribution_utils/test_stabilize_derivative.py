...
class TestClass(BaseTestClass):
    ...
    def test_stabilize_derivative_mixture(self, mixture_class, stabilization):
        # Create data for testing
        torch.manual_seed(123)
        input_der = torch.rand((10, 1), dtype=torch.float64)

        # Call the function
        stab_der = mixture_class.dist.stabilize_derivative(input_der, stabilization)
    ...
    def test_stabilize_derivative_mixture_nans(self, mixture_class, stabilization):
        # Create data for testing
        torch.manual_seed(123)
        input_der = torch.rand((10, 1), dtype=torch.float64)
        input_der[0] = torch.tensor([float("nan")])
