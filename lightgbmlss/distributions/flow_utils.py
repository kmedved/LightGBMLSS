...
def _flow_candidate_name(flow) -> str:
    flow_name = str(flow.__class__).split(".")[-1].split("'>")[0]
    flow_spec = f"(count_bins: {flow.count_bins}, order: {flow.order})"
    return flow_name + flow_spec


def _fit_flow_candidate(flow, target, max_iter: int) -> Dict[str, Any]:
    flow_name = _flow_candidate_name(flow)
    try:
        loss, params = flow.calculate_start_values(target=target, max_iter=max_iter)
        return {
            flow.loss_fn: float(np.asarray(loss).reshape(-1)[0]),
            "NormFlow": str(flow_name),
            "params": params,
        }
    except Exception as e:
        warnings.warn(f"Error fitting {flow} NormFlow: {str(e)}")
        return {
            flow.loss_fn: np.nan,
            "NormFlow": str(flow),
...
class NormalizingFlowClass:
    ...
    def get_params_loss(self,
        ...
        if self.loss_fn == "nll":
            ...
            with local_torch_seed(123):
                dist_samples = flow_dist.rsample((30,))
            dist_samples = dist_samples.reshape(dist_samples.shape[0], -1)
    ...
    def _draw_samples_array(self,
        ...
        flow_samples = flow_samples.reshape(n_samples, -1)
        return flow_samples.T
    ...
    def draw_samples(self,
        ...
        flow_samples = self._draw_samples_array(predt_params=predt_params,
                                                n_samples=n_samples,
                                                seed=seed)
        return _sample_array_to_frame(flow_samples, discrete=self.discrete)
    ...
    def predict_dist(self,
        ...
        if pred_type == "parameters":
            return pd.DataFrame(dist_params_predt, columns=self.param_dict.keys())
    ...
    def flow_select(self,
        ...
        n_jobs: int
            Number of parallel jobs used to fit candidate flows.
        ...
        if n_jobs == 1:
            ...
            with tqdm(total=total_iterations, desc="Fitting candidate normalizing flows") as pbar:
                for flow in candidate_flows:
                    ...
                    results.append(_fit_flow_candidate(flow, target, max_iter))
                    pbar.update(1)
                pbar.set_description("Fitting of candidate normalizing flows completed")
        else:
            results = Parallel(n_jobs=n_jobs, prefer="threads")(
                delayed(_fit_flow_candidate)(flow, target, max_iter)
                for flow in candidate_flows
            )

        fit_df = pd.DataFrame(results).sort_values(by=candidate_flows[0].loss_fn, ascending=True)
        fit_df["rank"] = fit_df[candidate_flows[0].loss_fn].rank().astype(int)
